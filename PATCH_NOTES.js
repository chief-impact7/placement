...
성적표 UI 레이아웃 및 텍스트 최적화
- 인적사항을 한 줄로 통합하고 학교/학년을 '선유고2' 형태로 결합하여 가독성을 높였습니다.
- '소속단지' 정보를 '소속' 라벨로 추가하고 불필요한 '과정' 및 영문 텍스트를 정리했습니다.
- 차트 영역의 크기를 확대하여 분석 데이터를 더 크게 볼 수 있도록 개선했습니다.
- 학원 로고의 시각적 선명도를 유지하고 하단 텍스트를 삭제하여 깔끔한 마감을 구현했습니다.
...
고등부 성적표 디자인 리뉴얼
- 고등부 전용 성적표의 가독성을 높이기 위해 차트 및 테이블 색상을 보강했습니다.
- 인적사항 레이아웃을 정보를 한눈에 보기 편하도록 재구성했습니다.
- 학원 공식 로고를 적용하고 불필요한 분석 문구 및 직인 영역을 삭제하여 깔끔한 디자인을 완성했습니다.
...
성적표 렌더링 오류 수정 (Hotfix)
- 성적표 열람 시 발생하던 화이트 스크린 오류를 해결했습니다. (아이콘 참조 및 데이터 전달 오류 수정)
...
고등부 전용 성적표(ReportCard) 출시
- 고등부 과정 학생들을 위한 전문적이고 아카데믹한 Slate 테마의 성적 분석표가 추가되었습니다.
- 고등부 핵심 6개 영역 및 SUM 데이터에 대한 비교 분석(내 점수, 상위 30%, 평균) 테이블과 시각화 차트가 제공됩니다.
- 초등/중등/고등부 통합 테마 시스템을 구축하여 학부별 맞춤형 디자인을 제공합니다.
...
학생 목록 정렬 순서 변경
- 학생 목록이 입력 순서의 역순으로 표시되도록 변경되었습니다. 이제 가장 최근에 입력한 학생의 정보가 목록 최상단에 나타납니다.
...
파일 이동 권한 및 로직 강화
- 내보내기 시 파일이 '내 드라이브'에 머무는 문제를 해결하기 위해 드라이브 접근 권한을 확장하고, 공유 드라이브 환경 호환성(supportsAllDrives)을 추가했습니다.
...
트렌드 차트 최적화
- 우측 성적 추세 분석 차트가 최대 4개까지만 표시되도록 제한하여 시스템 부하를 줄였습니다.
- 스크롤 시 차트 카드들이 서로 겹쳐서 가려지던 현상을 해결했습니다. (Sticky 레이아웃 수정)
